...
export const Room: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white flex flex-col font-sans selection:bg-emerald-500/30">
      <header className="h-16 border-b border-white/5 bg-black/40 backdrop-blur-xl px-6 flex items-center justify-between z-40 fixed top-0 w-full">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 px-4 py-1.5 bg-white/5 rounded-full border border-white/10 hover:bg-white/10 transition-colors cursor-pointer group" onClick={() => { navigator.clipboard.writeText(roomId!); alert('Code copied!'); }}>
            <span className="text-xs font-mono text-emerald-500 font-bold">{roomId}</span>
            <Share2 className="w-3.5 h-3.5 opacity-50 group-hover:opacity-100 transition-opacity" />
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          <div className="flex -space-x-3">
            {members.slice(0, 5).map((m, i) => (
              <div key={i} className="w-9 h-9 rounded-full bg-zinc-800 border-2 border-[#0a0a0a] flex items-center justify-center text-[10px] font-bold uppercase relative group" title={m.username}>
                {m.username[0]}
                <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 px-2 py-1 bg-zinc-900 border border-white/10 rounded text-[9px] whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                  {m.username} {m.role === 'host' && '(Host)'}
                </div>
              </div>
            ))}
            {members.length > 5 && <div className="w-9 h-9 rounded-full bg-zinc-900 border-2 border-[#0a0a0a] flex items-center justify-center text-[10px] font-bold">+{members.length - 5}</div>}
          </div>
          <button onClick={() => navigate('/')} className="p-2.5 hover:bg-red-500/10 hover:text-red-500 rounded-xl transition-all border border-transparent hover:border-red-500/20">
      ...
      <main className="flex-1 flex pt-16 overflow-hidden">
        {/* Left Sidebar */}
        <aside className="w-80 border-r border-white/5 bg-black/20 backdrop-blur-md p-6 flex flex-col gap-8">
        ...
        {/* Center Content */}
        <section className="flex-1 p-10 flex flex-col items-center justify-center relative">
          ...
          <div className="w-full max-w-5xl z-10">
            ...
            <div className="mt-8 flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-black tracking-tight bg-gradient-to-r from-white to-white/40 bg-clip-text text-transparent italic">NOW PLAYING</h1>
                <p className="text-zinc-500 text-sm font-medium mt-1 truncate max-w-lg">{videoUrl}</p>
                {currentVideoAddedBy && (
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2.5 px-4 py-2 bg-emerald-500/10 text-emerald-500 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-emerald-500/20 shadow-lg shadow-emerald-500/5">
